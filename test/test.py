...
class TestServer:
    ...
    def startServer(self):
        ...
        with open(os.devnull, 'w') as fp:
            ...
            self.server_process = subprocess.Popen(
                ["dev_appserver.py", "--datastore_path=" + self.temp_datastore_path, self.app_path,
                 "--clear_datastore"], stdout=fp, stderr=fp)
    ...
    def resume_server(self):
        self.server_process = subprocess.Popen(
            ["dev_appserver.py", "--datastore_path=" + temp_datastore_path, self.app_path], stdout=fp, stderr=fp)
        TestServer.wait_for_server()
...
def addInitialQuestion(duetime, question, numberQuestions, course, section):
    ...
    data = {"course": course.upper(), "section": section.upper(), "time": duetime, "question": question,
            "number": numberQuestions, "round": "1", "roundType": "initial", "startBuffer": "0", "options": options,
            "action": "add"}
...
def changeDiscussionRound(round_id, course, section, description, deadline, starttime):
    data = {"round_id": round_id, "course": course, "section": section, "description": description,
            "roundType": "discussion", "action": "change", "deadline": deadline, "start_time": starttime}
    return rounds_post(data)
...
def testGET(admin, instructor, student, course, section, situation_string):
    ...
    if instructor is not None:
        ...
        for dir in inst_directories:
            ...
            if not "200" in str(r):
                print("\t\t\tFAILED (expected 200): instructor got <" + str(
                    r) + "> when sending a GET request to <" + dir + "> when <" + situation_string + ">")
            else:
        ...
        for dir in admin_directories:
            r = session.get("http://localhost:8080" + dir)
            if not "401" in str(r):
                print("\t\t\tFAILED (expected 401): instructor got <" + str(
                    r) + "> when sending a GET request to <" + dir + "> when <" + situation_string + ">")
            else:
    ...
    if student is not None:
        ...
        for dir in std_directories:
            ...
            if dir == "/student_rounds":
                ...
                if len(keys) > 0:
                    ...
                    if not "200" in str(r):
                        print("\t\t\tFAILED (expected 200): student got <" + str(
                            r) + "> when sending a GET request to <" + dir + "> when <" + situation_string + ">")
                    else:
        ...
        for dir in inst_directories:
            ...
            if not ("302" in str(r.history) and "student_home" in r.url):
                print(
                "\t\t\tFAILED (expected 302 to student_home): url response was <" + r.url + ">, history was <" + str(
                    r.history) + "> when sending a GET request to <" + dir + "> when <" + situation_string + ">")
            else:
        ...
        for dir in admin_directories:
            r = session.get("http://localhost:8080" + dir)
            if not "401" in str(r):
                print("\t\t\tFAILED (expected 401): student got <" + str(
                    r) + "> when sending a GET request to <" + dir + "> when <" + situation_string + ">")
            else:
    ...
    if admin is not None:
        ...
        for dir in admin_directories:
            r = session.get("http://localhost:8080" + dir)
            if not "200" in str(r):
                print("\t\t\tFAILED (expected 200): admin got <" + str(
                    r) + "> when sending a GET request to <" + dir + "> when <" + situation_string + ">")
            else:
